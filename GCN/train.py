...
def train(args, rank):
    ...
    net=DDP(net,device_ids=[args.device])
    ...
    for iter_idx in range(1, args.train_max_iter):
        ...
        if rank==0:
            wandb.log({"Test RMSE":rmse/count_num})
            wandb.log({"Test iter time":np.average(dur)})
...
def config():
    ...
    parser.add_argument('--init', type=str, default="tcp://127.0.0.1:34444")
...
if __name__ == '__main__':
    ...
    args = config()
    args.device=rank#one machine and multi gpus
    th.cuda.set_device(args.device)
    th.distributed.init_process_group(backend='gloo', init_method=args.init, world_size=world_size, rank=rank)
