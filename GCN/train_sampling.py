...
def config():
    ...
    parser.add_argument('--init', type=str, default='tcp://127.0.0.1:33333')
    ...
    for epoch in range(1, args.train_max_epoch):
        ...
        for step, sample_data in enumerate(dataloader):           
            ...
            if iter_idx % args.train_log_interval == 0:
                ...
                if proc_id==0:
                    wandb.log({"Test loss":count_loss/iter_idx})
                    wandb.log({"Test RMSE":count_rmse/count_num})
                count_rmse = 0
                count_num = 0
        ...
        if epoch > 1:
            ...
            if proc_id==0:
                wandb.log({"Test epoch time":epoch_time})
                wandb.log({"Test throughput":datasize/epoch_time})
...
if __name__ == '__main__':
    os.environ["DGL_DOWNLOAD_DIR"]='/ibex/scratch/feij/sparse_model/GCN/dataset'
    args = config()
    ...
    # multi gpu
        ...
        run(rank, world_size, args, devices, dataset)
        '''
        procs = []
